<sub>A GitHub-native portfolio of public projects and selected private work.</sub>

## Featured Projects

This section is for larger, flagship, or especially representative work.

<table>
<tr>
<td width="38%">
<a href="projects/biometrics-dashboard.md">
<img src="assets/biometrics-dashboard/health-multi.webp" alt="Biometrics Dashboard showing multiple lab trend panels">
</a>
</td>
<td>

### Biometrics Dashboard

![Private](https://img.shields.io/badge/Private-&nbsp;-lightgrey)
![MVP](https://img.shields.io/badge/MVP-&nbsp;-blue)

Private case study. A single-user dashboard that turns lab PDFs into structured biometric trend data through an authenticated upload, parser, review, and import workflow.

It combines a plain PHP/JavaScript interface, MySQL persistence, TOTP-backed access control, and a Python extraction pipeline that uses PDF text/table extraction, OCR fallbacks, and configurable LLM parsing.

**Links:** [Case study](projects/biometrics-dashboard.md)

</td>
</tr>
</table>

## Recent Work

<table>
<tr>
<td width="38%">
<a href="https://github.com/robione-nr/utility-server">
<img src="assets/utility-server/admin-console.webp" alt="Utility Server admin console showing domain, TOTP, and rate-limit management">
</a>
</td>
<td>

### Utility Server

![Public](https://img.shields.io/badge/Public-&nbsp;-brightgreen)
![Stable](https://img.shields.io/badge/Stable-&nbsp;-brightgreen)

Public repo. A small FastAPI utility service I built for common cross-domain backend tasks: TOTP authentication, domain and policy management, token issuance, and database-backed rate limiting.

The project includes a lightweight PHP admin console for managing domains, TOTP policies, and rate-limit rules, with connectivity checks intended for web and mobile clients.

**Links:** [GitHub repo](https://github.com/robione-nr/utility-server)

</td>
</tr>
</table>

## Project Index

| Project | Visibility | Status | Summary | Links |
| --- | --- | --- | --- | --- |
| Biometrics Dashboard | ![Private](https://img.shields.io/badge/Private-&nbsp;-lightgrey) | ![MVP](https://img.shields.io/badge/MVP-&nbsp;-blue) | Authenticated lab-PDF parser and biomarker trend dashboard. | [Case study](projects/biometrics-dashboard.md) |
| Utility Server | ![Public](https://img.shields.io/badge/Public-&nbsp;-brightgreen) | ![Stable](https://img.shields.io/badge/Stable-&nbsp;-brightgreen) | FastAPI service and admin UI for TOTP, tokens, domains, and rate limiting. | [Repo](https://github.com/robione-nr/utility-server) |
